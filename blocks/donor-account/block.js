			...
			< >
				< InspectorControls >
					< PanelBody title  = {__( 'About', 'giftflow' )} initialOpen = {true} >
						< p style      = {{ color: '#757575', fontSize: 13, margin: 0 }} > {__( 'Displays the donor account interface with Dashboard, My Donations, and Account Settings tabs. Visitors see a login form if not authenticated.', 'giftflow' )} < / p >
					< / PanelBody >
				< / InspectorControls >
				< div {...blockProps} >
					< nav className    = "giftflow-donor-account__nav" role = "tablist" >
						{tabs.map(
							(tab, i) => (
					...
					< / nav >
					< div className                         = "giftflow-donor-account__content" style = {{ minHeight: 260 }} >
						< div style                         = {{ display: 'flex', flexDirection: 'column', gap: 16 }} >
							...
							< div style           = {{ border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }} >
								< div style       = {{ display: 'flex', backgroundColor: '#f9fafb', padding: '8px 14px', borderBottom: '1px solid #e5e7eb', gap: 12 }} >
									{[__( 'ID', 'giftflow' ), __( 'Amount', 'giftflow' ), __( 'Status', 'giftflow' ), __( 'Date', 'giftflow' )].map(
										(h, i) => (
								...
								{[1, 2, 3].map(
									r => (
									< div key         = {r} style = {{ display: 'flex', padding: '10px 14px', borderBottom: '1px solid #f3f4f6', gap: 12 }} >
										{[35, 55, 30, 45].map(
											(w, i) => (
